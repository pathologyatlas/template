


```{r language template, echo=FALSE, include=TRUE}
source("./R/language.R")
output_type <- knitr::opts_knit$get("rmarkdown.pandoc.to")
```


```{asis, echo = (language == "TR")}
## TemplateTR {#sec-template }
```


```{asis, echo = (language == "EN")}
## TemplateEN {#sec-template }
```





```{r template screenshot HE1, eval=TRUE, include=FALSE}
if (!file.exists("./screenshots/template-HE1_screenshot.png")) {
webshot2::webshot(
  url = "https://images.patolojiatlasi.com/template/HE1.html",
  file = "./screenshots/template-HE1_screenshot.png"
)
}
```



```{asis, echo = (language == "TR")}

**TemplateTR**


[![Tam Ekran Görmek İçin Resmi Tıklayın](./screenshots/template-HE1_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/HE1.html) [Tam Ekran Görmek İçin Resmi Tıklayın](https://images.patolojiatlasi.com/template/HE1.html)
```


```{asis, echo = ((language=="TR") & (output_type=="html"))}
Mikroskopik görüntüleri inceleyin:

<iframe src="https://images.patolojiatlasi.com/template/HE1.html" style="height:600px;width:100%;" data-external="1"></iframe>

```


```{asis, echo = (language == "EN")}

**TemplateEN**

[![Click for Full Screen WSI](./screenshots/template-HE1_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/HE1.html) [Click for Full Screen WSI](https://images.patolojiatlasi.com/template/HE1.html)

```



```{asis, echo = ((language == "EN") & (output_type=="html"))}

See Microscopy with viewer:

<iframe src="https://images.patolojiatlasi.com/template/HE1.html" style="height:600px;width:100%;" data-external="1"></iframe>

```





```{r template screenshot HE2, eval=TRUE, include=FALSE}
if (!file.exists("./screenshots/template-HE2_screenshot.png")) {
webshot2::webshot(
  url = "https://images.patolojiatlasi.com/template/HE2.html",
  file = "./screenshots/template-HE2_screenshot.png"
)
}
```



```{asis, echo = (language == "TR")}

**TemplateTR**


[![Tam Ekran Görmek İçin Resmi Tıklayın](./screenshots/template-HE2_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/HE2.html) [Tam Ekran Görmek İçin Resmi Tıklayın](https://images.patolojiatlasi.com/template/HE2.html)
```


```{asis, echo = ((language=="TR") & (output_type=="html"))}
Mikroskopik görüntüleri inceleyin:

<iframe src="https://images.patolojiatlasi.com/template/HE2.html" style="height:600px;width:100%;" data-external="1"></iframe>

```


```{asis, echo = (language == "EN")}

**TemplateEN**

[![Click for Full Screen WSI](./screenshots/template-HE2_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/HE2.html) [Click for Full Screen WSI](https://images.patolojiatlasi.com/template/HE2.html)

```



```{asis, echo = ((language == "EN") & (output_type=="html"))}

See Microscopy with viewer:

<iframe src="https://images.patolojiatlasi.com/template/HE2.html" style="height:600px;width:100%;" data-external="1"></iframe>

```





```{r template screenshot TR, eval=TRUE, include=FALSE}
if (!file.exists("./screenshots/template-TR_screenshot.png")) {
webshot2::webshot(
  url = "https://images.patolojiatlasi.com/template/TR.html",
  file = "./screenshots/template-TR_screenshot.png"
)
}
```



```{asis, echo = (language == "TR")}

**TemplateTR**


[![Tam Ekran Görmek İçin Resmi Tıklayın](./screenshots/template-TR_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/TR.html) [Tam Ekran Görmek İçin Resmi Tıklayın](https://images.patolojiatlasi.com/template/TR.html)
```


```{asis, echo = ((language=="TR") & (output_type=="html"))}
Mikroskopik görüntüleri inceleyin:

<iframe src="https://images.patolojiatlasi.com/template/TR.html" style="height:600px;width:100%;" data-external="1"></iframe>

```


```{asis, echo = (language == "EN")}

**TemplateEN**

[![Click for Full Screen WSI](./screenshots/template-TR_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/TR.html) [Click for Full Screen WSI](https://images.patolojiatlasi.com/template/TR.html)

```



```{asis, echo = ((language == "EN") & (output_type=="html"))}

See Microscopy with viewer:

<iframe src="https://images.patolojiatlasi.com/template/TR.html" style="height:600px;width:100%;" data-external="1"></iframe>

```





```{r template screenshot ER, eval=TRUE, include=FALSE}
if (!file.exists("./screenshots/template-ER_screenshot.png")) {
webshot2::webshot(
  url = "https://images.patolojiatlasi.com/template/ER.html",
  file = "./screenshots/template-ER_screenshot.png"
)
}
```



```{asis, echo = (language == "TR")}

**TemplateTR**


[![Tam Ekran Görmek İçin Resmi Tıklayın](./screenshots/template-ER_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/ER.html) [Tam Ekran Görmek İçin Resmi Tıklayın](https://images.patolojiatlasi.com/template/ER.html)
```


```{asis, echo = ((language=="TR") & (output_type=="html"))}
Mikroskopik görüntüleri inceleyin:

<iframe src="https://images.patolojiatlasi.com/template/ER.html" style="height:600px;width:100%;" data-external="1"></iframe>

```


```{asis, echo = (language == "EN")}

**TemplateEN**

[![Click for Full Screen WSI](./screenshots/template-ER_screenshot.png){width="25%"}](https://images.patolojiatlasi.com/template/ER.html) [Click for Full Screen WSI](https://images.patolojiatlasi.com/template/ER.html)

```



```{asis, echo = ((language == "EN") & (output_type=="html"))}

See Microscopy with viewer:

<iframe src="https://images.patolojiatlasi.com/template/ER.html" style="height:600px;width:100%;" data-external="1"></iframe>

```


